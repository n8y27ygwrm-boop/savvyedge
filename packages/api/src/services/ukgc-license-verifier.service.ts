 ...
 */
export const defaultUkgcDatasetFetcher: UkgcDatasetFetcher =
  async (): Promise<UkgcDatasets> => {
    const headers = {
      "User-Agent":
...
export class UkgcLicenseVerifierService {
  ...
  public static async verifyCasinoLicense(
    ...
    if (
      !db ||
      "$connect" in (db as object) ||
      "$disconnect" in (db as object)
    ) {
      throw new Error(
    ...
    // 3. Persist only through the exact transaction supplied by the guarded
    // bootstrap. This kernel never selects or opens an ambient connection.
    return this.persistAuthoritativeLicense(db, casinoId, resolved, now);
  }
  ...
  private static async persistAuthoritativeLicense(
    ...
    // Machine verification can only submit evidence for human review.
    if (finalReviewStatus === ReviewStatus.NEW) {
      const subRes = await workflowService.transitionLicenseReview({
        ...
        claimIds,
        internalReason:
          "Authoritative UKGC verification discovered valid active licence",
      });
