...
import { UKGC_DATASET_URLS } from "../../src/services/ukgc-license-verifier.service";
...
export async function executeD2AcceptanceRunner(
  ...
  log(
    ` [PASS] Isolated database confirmed: '${safety.databaseName}' on host '${safety.hostname}'\n`,
  );
  ...
  const originalCasinoAgentRun = (IngestionService as any).casinoResolutionAgent
    .run;
  ...
  try {
    ...
    (IngestionService as any).casinoResolutionAgent.run =
      originalCasinoAgentRun;
  ...
  if (!initialBonus) {
    throw new Error(
      `Ingestion failed: No bonus linked to Casino ${initialCasino.id}`,
    );
  }

  // Assert Stage 2 Casino properties
  if (initialCasino.status !== "ACTIVE")
    throw new Error("Casino status must be ACTIVE");
  if (initialCasino.verified_at !== null)
    throw new Error("Casino verified_at must be null at Stage 2");
  if (initialCasino.review_status !== ReviewStatus.AWAITING_REVIEW) {
    throw new Error(
      `Casino review_status must be AWAITING_REVIEW, got ${initialCasino.review_status}`,
    );
  }
  ...
  if (initialCasino.governance_version !== 1) {
    throw new Error(
      `Casino governance_version must be 1, got ${initialCasino.governance_version}`,
    );
  }

  // Assert Stage 2 Bonus properties
  if (initialBonus.status !== "ACTIVE")
    throw new Error("Bonus status must be ACTIVE");
  if (initialBonus.verified_at !== null)
    throw new Error("Bonus verified_at must be null at Stage 2");
  if (initialBonus.review_status !== ReviewStatus.AWAITING_REVIEW) {
    throw new Error(
      `Bonus review_status must be AWAITING_REVIEW, got ${initialBonus.review_status}`,
    );
  }
  ...
  if (initialBonus.governance_version !== 1) {
    throw new Error(
      `Bonus governance_version must be 1, got ${initialBonus.governance_version}`,
    );
  }
  ...
  if (!operatorEvidence)
    throw new Error("Operator page EvidenceRecord not found");

  const casinoClaims = await prisma.casinoEvidenceClaim.findMany({
    where: { casino_id: initialCasino.id },
  });
  if (casinoClaims.length === 0)
    throw new Error("No CasinoEvidenceClaim records found");

  const bonusClaims = await prisma.bonusEvidenceClaim.findMany({
    where: { bonus_id: initialBonus.id },
  });
  if (bonusClaims.length === 0)
    throw new Error("No BonusEvidenceClaim records found");
  ...
  if (!casinoReviewAudit)
    throw new Error("Casino REVIEW_REQUESTED audit event missing");
  ...
  if (!bonusReviewAudit)
    throw new Error("Bonus REVIEW_REQUESTED audit event missing");

  log(
    ` [PASS] Governed ingestion created Casino (${initialCasino.id}) and Bonus (${initialBonus.id})\n`,
  );
  ...
  const ukgcResult = await runUkgcLicenseBootstrap(
    { casinoId: initialCasino.id, execute: true },
    {
      database: prisma,
      fetcher: async () => deterministicUkgcDatasets,
      provePersistenceConnection: async () => ({
        proven: true,
        role: "savvyedge_d2_test",
        databaseName: "savvyedge_d2_test",
      }),
    },
  );

  if (ukgcResult.status !== "BOOTSTRAPPED") {
    throw new Error(`UKGC verification failed: ${ukgcResult.status}`);
  }

  // Assert synthetic account provenance (must not reuse real D1 account 45322)
  if (deterministicUkgcDatasets.domainsCsv.includes('"45322"')) {
    throw new Error("UKGC verification must not use real D1 account 45322");
  }
  if (!deterministicUkgcDatasets.domainsCsv.includes(`"${ukgcAccount}"`)) {
    throw new Error(
      `UKGC verification fixture must contain synthetic account '${ukgcAccount}'`,
    );
  }
  if (!deterministicUkgcDatasets.businessesCsv.includes(ukgcBusinessName)) {
    throw new Error(
      `UKGC verification fixture must contain '${ukgcBusinessName}'`,
    );
  }
  const verifiedCasino = await prisma.casino.findUniqueOrThrow({
    where: { id: initialCasino.id },
  });
  const verifiedLicense = await prisma.license.findUniqueOrThrow({
    where: { id: ukgcResult.licenseId },
  });

  if (verifiedLicense.license_no !== ukgcLicenceNo) {
    throw new Error(
      `License record license_no must match '${ukgcLicenceNo}', got '${verifiedLicense.license_no}'`,
    );
  }

  if (!verifiedCasino.verified_at)
    throw new Error(
      "Casino verified_at must be populated after UKGC verification",
    );
  ...
  if (verifiedCasino.governance_version !== 1) {
    throw new Error(
      "Casino governance_version must remain unchanged at Stage 3",
    );
  }
  ...
  if (!casinoHistory)
    throw new Error("VERIFICATION CasinoHistoryEvent missing");
  if (casinoHistory.source_url !== UKGC_DATASET_URLS.domains) {
    throw new Error(
      `CasinoHistoryEvent source_url must be domains register, got ${casinoHistory.source_url}`,
    );
  }

  if (verifiedLicense.status !== "ACTIVE")
    throw new Error("License status must be ACTIVE");
  if (!verifiedLicense.verified_at)
    throw new Error("License verified_at must be populated");
  ...
  if (licenseClaims.length !== 4) {
    throw new Error(
      `Expected exactly 4 LicenseEvidenceClaims, got ${licenseClaims.length}`,
    );
  }

  log(
    ` [PASS] UKGC verification created License (${verifiedLicense.id}) and verified Casino\n`,
  );
  ...
  const approvedLicense = await prisma.license.findUniqueOrThrow({
    where: { id: verifiedLicense.id },
  });
  ...
  if (approvedLicense.governance_version !== 3) {
    throw new Error(
      `License governance_version must be 3, got ${approvedLicense.governance_version}`,
    );
  }
  ...
  if (licApproveAudit.actor_id !== humanActor.id)
    throw new Error("License approval audit actor must be human actor");
  if (licApproveAudit.evidence_claims.length === 0)
    throw new Error("License approval claims must be linked to audit event");

  log(
    ` [PASS] Human actor (${humanActor.id}) approved License (${approvedLicense.id})\n`,
  );
  ...
  const machineVerifiedBonus = await prisma.bonus.findUniqueOrThrow({
    where: { id: initialBonus.id },
  });
  if (machineVerifiedBonus.status !== "ACTIVE")
    throw new Error("Bonus status must be ACTIVE");
  if (!machineVerifiedBonus.verified_at)
    throw new Error("Bonus verified_at must be populated after VALIDATE_BONUS");
  ...
  if (
    machineVerifiedBonus.publication_status !== PublicationStatus.UNPUBLISHED
  ) {
    throw new Error("Bonus publication_status must remain UNPUBLISHED");
  }
  if (machineVerifiedBonus.governance_version !== 1) {
    throw new Error(
      "Bonus governance_version must remain unchanged at Stage 5",
    );
  }
  ...
  if (!bonusVerificationHistory)
    throw new Error("verified_at BonusHistoryEvent missing");
  if (bonusVerificationHistory.source_url !== offerUrl) {
    throw new Error(
      `BonusHistoryEvent source_url must match offerUrl, got ${bonusVerificationHistory.source_url}`,
    );
  }
  ...
  const casinoEvidencePre =
    PublicationGateService.getQualifyingCasinoEvidence(prePubCasino);
  const bonusEvidencePre =
    PublicationGateService.getQualifyingBonusEvidence(prePubBonus);

  if (!casinoEvidencePre)
    throw new Error("Qualifying Casino evidence must not be null");
  if (!bonusEvidencePre)
    throw new Error("Qualifying Bonus evidence must not be null");
  ...
  log(
    " [PASS] PublicationGate fails closed prior to human approval and publication\n",
  );
  ...
  const approvedCasino = await prisma.casino.findUniqueOrThrow({
    where: { id: initialCasino.id },
  });
  if (approvedCasino.review_status !== ReviewStatus.APPROVED)
    throw new Error("Casino review_status must be APPROVED");
  if (approvedCasino.publication_status !== PublicationStatus.UNPUBLISHED) {
    throw new Error("Casino publication_status must remain UNPUBLISHED");
  }
  if (approvedCasino.governance_version !== 3) {
    throw new Error(
      `Casino governance_version must be 3, got ${approvedCasino.governance_version}`,
    );
  }
  ...
  if (casApproveAudit.actor_id !== humanActor.id)
    throw new Error("Casino approval actor must be human actor");
  if (casApproveAudit.evidence_claims.length === 0)
    throw new Error("Casino approval claims must be linked");
  ...
  const activeBonusPointer =
    await prisma.activeExtractionPointer.findUniqueOrThrow({
      where: {
  ...
  const activeBonusClaims = await prisma.bonusEvidenceClaim.findMany({
    where: {
      bonus_id: initialBonus.id,
      evidence_id: activeBonusPointer.evidence_id,
    },
    select: { id: true },
  });
  const bonusClaimIds = activeBonusClaims.map((claim) => claim.id);
  ...
  const approvedBonus = await prisma.bonus.findUniqueOrThrow({
    where: { id: initialBonus.id },
  });
  if (approvedBonus.review_status !== ReviewStatus.APPROVED)
    throw new Error("Bonus review_status must be APPROVED");
  if (approvedBonus.publication_status !== PublicationStatus.UNPUBLISHED) {
    throw new Error("Bonus publication_status must remain UNPUBLISHED");
  }
  if (approvedBonus.governance_version !== 3) {
    throw new Error(
      `Bonus governance_version must be 3, got ${approvedBonus.governance_version}`,
    );
  }
  ...
  if (bonApproveAudit.actor_id !== humanActor.id)
    throw new Error("Bonus approval actor must be human actor");
  if (bonApproveAudit.evidence_claims.length === 0)
    throw new Error("Bonus approval claims must be linked");
  ...
  const publishedCasino = await prisma.casino.findUniqueOrThrow({
    where: { id: initialCasino.id },
  });
  ...
  if (publishedCasino.governance_version !== 4) {
    throw new Error(
      `Casino governance_version must be 4, got ${publishedCasino.governance_version}`,
    );
  }
  ...
  if (casPubAudit.event_type !== WorkflowEventType.PUBLISHED)
    throw new Error("Audit event must be PUBLISHED");
  if (casPubAudit.actor_id !== humanActor.id)
    throw new Error("Audit actor must be human actor");

  // Verify Casino claims linked, but License claims are NOT linked to Casino publication event
  const casPubClaimIds = casPubAudit.evidence_claims
    .map((c) => c.casino_evidence_claim_id)
    .filter(Boolean);
  const licPubClaimIds = casPubAudit.evidence_claims
    .map((c) => c.license_evidence_claim_id)
    .filter(Boolean);

  for (const cc of casinoClaimIds) {
    if (!casPubClaimIds.includes(cc))
      throw new Error(`Casino claim ${cc} missing from publication event`);
  }
  if (licPubClaimIds.length > 0) {
    throw new Error(
      "License claims must not be linked to Casino publication event",
    );
  }

  log(
    ` [PASS] Casino (${publishedCasino.id}) published (version: ${publishedCasino.governance_version})\n`,
  );
  ...
  const publishedBonus = await prisma.bonus.findUniqueOrThrow({
    where: { id: initialBonus.id },
  });
  ...
  if (publishedBonus.governance_version !== 4) {
    throw new Error(
      `Bonus governance_version must be 4, got ${publishedBonus.governance_version}`,
    );
  }
  ...
  if (bonPubAudit.event_type !== WorkflowEventType.PUBLISHED)
    throw new Error("Audit event must be PUBLISHED");
  if (bonPubAudit.actor_id !== humanActor.id)
    throw new Error("Audit actor must be human actor");

  const bonPubClaimIds = bonPubAudit.evidence_claims
    .map((c) => c.bonus_evidence_claim_id)
    .filter(Boolean);
  for (const bc of bonusClaimIds) {
    if (!bonPubClaimIds.includes(bc))
      throw new Error(`Bonus claim ${bc} missing from publication event`);
  }

  log(
    ` [PASS] Bonus (${publishedBonus.id}) published (version: ${publishedBonus.governance_version})\n`,
  );
  ...
  if (PublicationGateService.isCasinoPubliclyEligible(finalCasino) !== true) {
    throw new Error(
      "Final published Casino must pass PublicationGateService.isCasinoPubliclyEligible",
    );
  }
  if (
    PublicationGateService.isBonusPubliclyEligible(finalBonus, finalCasino) !==
    true
  ) {
    throw new Error(
      "Final published Bonus must pass PublicationGateService.isBonusPubliclyEligible",
    );
  }

  const finalCasinoEvidence =
    PublicationGateService.getQualifyingCasinoEvidence(finalCasino);
  if (!finalCasinoEvidence)
    throw new Error("Final Casino qualifying evidence missing");
  if (finalCasinoEvidence.event_type !== "VERIFICATION") {
    throw new Error(
      `Casino qualifying evidence event_type must be VERIFICATION, got ${finalCasinoEvidence.event_type}`,
    );
  }
  if (finalCasinoEvidence.source_url !== UKGC_DATASET_URLS.domains) {
    throw new Error(
      `Casino qualifying evidence source_url must match domains register, got ${finalCasinoEvidence.source_url}`,
    );
  }
  if (
    new Date(finalCasinoEvidence.occurred_at).getTime() !==
    new Date(finalCasino.verified_at!).getTime()
  ) {
    throw new Error(
      "Casino qualifying evidence occurred_at must correlate exactly with Casino.verified_at",
    );
  }

  const finalBonusEvidence =
    PublicationGateService.getQualifyingBonusEvidence(finalBonus);
  if (!finalBonusEvidence)
    throw new Error("Final Bonus qualifying evidence missing");
  if (finalBonusEvidence.field_changed !== "verified_at") {
    throw new Error(
      `Bonus qualifying evidence field_changed must be verified_at, got ${finalBonusEvidence.field_changed}`,
    );
  }
  if (
    new Date(finalBonusEvidence.new_value).getTime() !==
    new Date(finalBonus.verified_at!).getTime()
  ) {
    throw new Error(
      "Bonus verification history new_value must equal the authoritative observed_at projection",
    );
  }
  if (finalBonusEvidence.source_url !== offerUrl) {
    throw new Error(
      `Bonus qualifying evidence source_url must match offerUrl, got ${finalBonusEvidence.source_url}`,
    );
  }

  log(
    " [PASS] PublicationGate verified: both Casino and Bonus are publicly eligible with exact provenance!\n",
  );
